 ...
 * @author El Jefe
 */
@WebServlet(name = "BannerBoardUpdateServlet", urlPatterns = {"/BannerBoardUpdateServlet"})
public class BannerBoardUpdateServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet BannerBoardUpdateServlet</title>");            
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet BannerBoardUpdateServlet at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
    ...
    
    {
            ...
            
        {
                ...
                
            {
                ...
                System.out.println("Trying to upload " + directoryName + " to board " + boardID);
                
                if (boardID != -1)
                    
                {
                    s3Client.uploadfile(credentials, bucketName, directoryName, fileIS, metadata);
                    uploadedPicture = "https://s3.ca-central-1.amazonaws.com/pegbucket/boards/wallpapers/" + fileName;
                    update = new BoardUpdate(boardID);
                    update.setBoardPoster(uploadedPicture);
